...
function carica_xml() {
    ...
    $.get("xml/lista.xml", {}, function(data) {
        ...
        $(data).find("pubblicazione").each(function() {
            ...
            var sottotitolo = $(this).children("sottotitolo").text();
...
function crea_scheda(num_scheda){
    $.get("xml/lista.xml", {}, function(data) {
        ...
        $(data).find("pubblicazione:eq("+num_scheda+")").each(function(){
            ...
            var data_pubblicazione = $(this).children("data_pubblicazione").text();
            ...
            var presentazione = elabora_nodo($(this).children("presentazione"));
            var destinatari  = elabora_nodo($(this).children("destinatari"));
            var indice  = elabora_nodo($(this).children("indice"));
            var autori  = elabora_autori($(this).children("autori"));
            var recensioni  = elabora_nodo($(this).children("recensioni"));
            ...
            output.push("<div class='box_dati'>");
                ...
                output.push("<div class='ui-grid-a'>");
                    output.push("<div class='ui-block-a'>");
                        ...
                        output.push("<p>Costo: <b>"+costo+" €</b></p>");
                        output.push("<p>Formato: <b>"+formato+" cm</b></p>");
                        output.push("<p><b>"+n_pagine+" pagine</b></p>");
                    output.push("</div>");
                    output.push("<div class='ui-block-b'>");
                    if (ISBN_ebook.length>0) {
                        output.push("<h4>E-book</h4>");
                        output.push("<p>ISBN: <b>"+ISBN_ebook+"</b></p>");
                        output.push("<p>Costo: <b>"+costo_ebook+" €</b></p>");
...
function elabora_autori(campo_autori){          
    ...
    for (p=0; p<nodi_autore.length; p++){
        testo_autori.push("<p><b>"+nodi_autore.eq(p).children("autore_nome").text()+"</b>. "+nodi_autore.eq(p).children("autore_affiliazione").text()+"</p>");
    }
    if (campo_autori.children("autori_note").text().length>0) {
         testo_autori.push("<p><i>"+campo_autori.children("autori_note").text()+"</i></p>");
    }
